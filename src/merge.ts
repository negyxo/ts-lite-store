...
/**
 * Deep merges two objects into the one. If the same properties are present in
 * both objects, then first one is overwriten with second.
...
function mergeInternal(first: any, second: any): { obj: any, deepSame: boolean } {
    ...
    // flag that indicates whether the properties are all the same in the second
    // object. If this is the case, we return the second object instead of newly
    // generated one. In this way we are allowing objects to be used by
    // refrenece in some simple casses. For instance if we have an array of
    // objects, and we want to store some element externally, if we use the
    // merge function, storing external value would generate a new object, and
    // we won't be able to use reference to compare objects, which is sometimes
    // faster way of comparing two objects
    let allSame = true;
